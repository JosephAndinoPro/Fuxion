...
const App: React.FC = () => {
    ...
    try {
      ...
      try {
        if (!process.env.API_KEY) {
          console.warn("API_KEY environment variable is not set. Skipping lifestyle tips generation.");
          lifestyleTips = "Lifestyle tips generation is unavailable (API key missing). Please consult your wellness advisor for personalized lifestyle advice.";
        } else {
  ...
  return (
    <div className={`min-h-screen w-full bg-gradient-to-br ${currentGradient} p-4 sm:p-6 md:p-8 flex flex-col items-center transition-all duration-500 ease-in-out`}>
      ...
      <main className="w-full max-w-5xl bg-white/90 backdrop-blur-lg shadow-2xl rounded-xl p-6 sm:p-8 md:p-10 transform transition-all duration-500 ease-in-out"> {/* Increased max-w-4xl to max-w-5xl */}
        {isLoading && (
